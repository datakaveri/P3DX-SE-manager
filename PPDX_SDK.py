...
def generate_guest_report():
    ...
    # Create the vm-files directory if it doesn't exist
    directory = "vm-files"
    if not os.path.exists(directory):
        os.makedirs(directory)
    
    # Change the current working directory to vm-files
    os.chdir(directory)
    
    # Execute the tpm2_nvread command
    try:
        subprocess.run(
            ["sudo", "tpm2_nvread", "-C", "o", "0x01400001", "-s", "0", "-o", "./snp_report.bin"],
            check=True
        )
        print("tpm2_nvread command executed successfully")
    except subprocess.CalledProcessError as e:
        print(f"Error executing tpm2_nvread command: {e}")
        return
    ...
    try:
        ...
        print("dd command executed successfully")
        # Change directory back to the original directory
        os.chdir(script_dir)
    except subprocess.CalledProcessError as e:
        print(f"Error executing dd command: {e}")
        # Change directory back to the original directory
        os.chdir(script_dir)
        return
...
def generate_tpm_quote():
    ...
    print("Executing tpm2_quote command...")
    try:
        subprocess.run(
            ["tpm2_quote", "-c", "0x81000003", "-l", "sha256:0,1,2,3,4,5,6,7,8,15", 
             "-m", "message_output_file.msg", "-s", "signature_output_file.sig", "-o", "PCR_output_file.pcrs", "-g", "sha256"],
            check=True
        )
        print("tpm2_quote command executed successfully.")
        print("TPM quote generated containing pcr 0-15")
        print("Binary file message_output_file.msg, signature_output_file.sig, and PCR_output_file.pcrs have been generated for verification purposes")
        os.chdir(script_dir)
    except subprocess.CalledProcessError:
        print("Error: Failed to execute tpm2_quote command.")
        os.chdir(script_dir)
        return

def create_json_from_folder():
    folder_path = 'vm-files'
        ...
        if os.path.isfile(file_path):
            try:
                with open(file_path, "rb") as f:
                    ...
                    binary_data[file_name] = encoded_content
            except FileNotFoundError:
                print(f"File not found: {file_path}")
                continue
    with open('keys/public_key.pem', 'rb') as f:
        public_key = f.read()
        encoded_content = base64.b64encode(public_key).decode('utf-8')
        binary_data['public_key.pem']=encoded_content  

    # Write the JSON data to a file
    with open(json_file_path, "w") as json_file:
        json.dump(binary_data, json_file, indent=4)
...
#APD verifies quote and releases token
def getTokenFromAPD_noMAA(bin_file,config_file):
