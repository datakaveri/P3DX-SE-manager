...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Error: Missing GitHub raw link argument.", flush=True)
        print("Usage: sudo python3 deploy_enclave.py <github_raw_link>", flush=True)
        sys.exit(1)  # Exit with an error code

    github_raw_link = sys.argv[1]
    if not github_raw_link.startswith("https://raw.githubusercontent.com/"):
        print("Error: Invalid GitHub raw link format.", flush=True)
        sys.exit(1)
