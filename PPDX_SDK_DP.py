...
def getFileFromResourceServer(token):                                       #TODO       
...
def pullconfig_SKALD(url, token, key):
    print("Pulling SKALD application config from RS..")
    access_token=token
    rs_url=url
    ...
    if response.status_code == 200: 
        ...
        config_path = os.path.expanduser("/tmp/SKALD_input/config")
        config_file = os.path.join(config_path, "config.json")

        with open(config_file, 'w') as json_file:
            json.dump(decryptedConfigDict, json_file, indent=4)
        print("Decrypted config written to tmp/SKALD_input/config")

def dataChunkN_SKALD(n, url, access_token, key):
    count=n
    loadedDict=getChunkFromResourceServer(count, url, access_token)
    if loadedDict:
        decryptChunk_SKALD(loadedDict, count, key)
        return 1
    else:
        return 0
    
def decryptChunk_SKALD(loadedDict, n, key):
    print("Decrypting chunk..")
    b64encryptedKey=loadedDict["encryptedKey"]
    encData=loadedDict["encData"]
    encryptedKey=base64.b64decode(b64encryptedKey)
    ...
    temp_dir = os.path.expanduser("/tmp/SKALD_input")
    decrypted_data_folder = os.path.join(temp_dir, "encrypted_data")
    extracted_data_folder = os.path.join(temp_dir, "input_file")
...
def pullCSVData_SKALD(url, access_token, key):
    """Pull CSV data directly from resource server without chunking."""
    print("Pulling CSV data directly from RS..")
    
    try:
        # Fetch encrypted data from resource server
        rs_headers = {'Authorization': f'Bearer {access_token}'}
        rs_url = url.rstrip('/')
        
        print(f"Accessing: {rs_url}")
        response = requests.get(rs_url, headers=rs_headers)
        ...
        print("Token authenticated and encrypted data received.")
        loadedDict = pickle.loads(response.content)
        ...
        # Decompress gzipped data in-memory (no intermediate file)
        with gzip.open(io.BytesIO(decryptedData), 'rb') as gz_file:
            csv_data = gz_file.read().decode('utf-8')
        
        # Ensure output directory exists and save CSV
        output_dir = os.path.expanduser("/tmp/SKALD_input/input_file")
        os.makedirs(output_dir, exist_ok=True)
        
        outfile_path = os.path.join(output_dir, "data.csv")
        with open(outfile_path, 'w', newline='', encoding='utf-8') as csvfile:
            csvfile.write(csv_data)
        
        print(f"CSV data saved to {outfile_path}")
        return True
        
    except Exception as e:
        print(f"Error in pullCSVData_SKALD: {str(e)}")
        return False

def encryptInference_SKALD(inference_key):
    print("Encrypting inference")
    output_file = os.path.expanduser("/tmp/SKALD_output/response.json")
    config_dir = os.path.expanduser("/tmp/SKALD_input/config")

    if not os.path.exists(output_file):
        concat_output = {"status": "empty", "message": "No inference output found"}
    else:
        with open(output_file, 'r') as f:
            concat_output = json.load(f)
    ...
    inference_file = os.path.expanduser("/tmp/SKALD_output/inference.json")
    os.rename(output_file, inference_file)
    print(f"File renamed and saved as {inference_file}")
